...
void  RmsNorm(float *out, const float *x, const std::bfloat16_t *weights, float eps, int n, int batch_size = 1);
void  Softmax(float *out, const float *x, int n);
float Silu(float x);
void  MatMul(float *out, const float *x, const std::bfloat16_t *y, int n, int m, int batch_size = 1);
void  ApplyRotaryEmb(float *out, int d, int head_dim, int pos, float theta, int rotary_dim);
void  FeedForwardNetwork(float *out, float *lin1, float *lin2, const float *x, const std::bfloat16_t *w1, const std::bfloat16_t *w2, const std::bfloat16_t *w3, int hidden_dim, int dim, int batch_size = 1);
