...
CPUImpl::CPUImpl(const std::string& path, int context_length)
    : Model(path) {
    InitializeInference(context_length);
}

auto CPUImpl::InitializeBackend() -> void {
...
auto CPUImpl::ResetBackend() -> void {
...
auto CPUImpl::ForwardToken(std::int32_t token, int pos, State &state) ->  void {
    const auto& config = GetInferenceConfig();
...
) -> void 
{
    ...
    // basically extend ForwardToken to consider another dimension
    const auto& config = GetInferenceConfig();
    ...
    if (start > context_length || num_tokens > context_length - start) {
        throw std::out_of_range(
            "Prefill chunk exceeds maximum context length"
        );
    }
    ...
    for (auto t{0uz}; t < num_tokens; t++) {
        ...
        const auto* embedding_row =
            embedding_.data()
            + static_cast<size_t>(token) * config.dim;

        auto* dst =
            hidden_state_.data()
            + static_cast<size_t>(t) * config.dim;
    ...
    const auto* last_hidden =
        normalized_state_.data()
        + static_cast<size_t>(num_tokens - 1) * config.dim;
    MatMul(
        logits_.data(),
        last_hidden,
        classifier,
...
) -> GenerationResult
{
    ...
    State state(&GetInferenceConfig());
    // [tokens, dim]
    std::span<const std::int32_t> tokens{prompt_tokens};
    for (auto i{0uz}; i < prompt_tokens.size(); i += batch_size) {
        auto batch_tokens = tokens.subspan(i, std::min(batch_size, prompt_tokens.size() - i));
        Prefill(batch_tokens, static_cast<int>(i), state);
    }
